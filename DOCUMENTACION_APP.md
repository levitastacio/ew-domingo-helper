# EasyWorship Domingo Helper - Documentación

## 📋 RESUMEN EJECUTIVO

- **Proyecto:** EasyWorship Domingo Helper
- **Framework:** React 18 + Vite
- **Almacenamiento:** ✅ SOLO localStorage (SIN Supabase)
- **Persistencia:** Datos guardados en el navegador del cliente
- **Líneas de código:** 605 líneas en App.jsx

---

## 🔍 ESTADO DEL ALMACENAMIENTO

### ✅ CONFIRMADO: NO TIENE SUPABASE
- No hay imports de Supabase
- No hay API calls a backend
- No hay archivo `supabase.js`
- No hay variables de entorno de Supabase

### ✅ USA localStorage PARA:
1. **Canciones guardadas** - `localStorage.getItem('ew_songs')`
2. **Checklist de domingo** - `localStorage.getItem('ew_checklist')`
3. **Datos del servicio** - Se generan en memoria y se descargan como .txt
...
ew-domingo-helper/
├── src/
│   ├── App.jsx           ← Componente principal (605 líneas)
│   ├── App.css           ← Estilos (400+ líneas)
│   ├── main.jsx          ← Punto de entrada
│   └── index.html
├── public/
│   └── logo-ebs.png      ← Logo de la iglesia
├── package.json
├── vite.config.js
└── DOCUMENTACION_APP.md  ← Este archivo
```

---

## 💾 CONTENIDO COMPLETO DE src/App.jsx
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const savedChecklist = localStorage.getItem('ew_checklist');
    if (savedChecklist) {
  ...
  const saveChecklist = (updatedChecklist) => {
    ...
    localStorage.setItem('ew_checklist', JSON.stringify(updatedChecklist));
  };
          ...
          <section className="tab-section">
            ...
            <div className="library-container">
              <div className="add-song-panel">
                ...
                <div className="form-group">
                  <label>Letra (pega aquí, usa [VERSO], [CORO], etc.)</label>
                  <textarea
                    value={newSongLyrics}
...
| **Almacenamiento de archivos** | ❌ localStorage (no archivos reales) |

## 📝 ACCIONES DEL USUARIO GUARDADAS EN localStorage
...
```javascript
{
  ...
  "ew_checklist": {
    "Revisar conexión de audio y micrófono": true,
    "Cargar tema gráfico de EasyWorship": false,
    ...
  }
}
```
...
**Documento creado el:** 2026-05-02
**Para compartir con:** Claude en próximas conversaciones
